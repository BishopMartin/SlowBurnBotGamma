...
export default function DashboardPage() {
  ...
  function SortTh({ label, field, className = "" }: { label: string; field: SortKey; className?: string }) {
    ...
    return (
      <th
        className={`px-[5px] py-2 font-normal cursor-pointer select-none transition-colors hover:text-[#f4f3ee] ${active ? "text-[#d97757]" : ""} ${className}`}
          ...
          <table className="w-full">
            <thead>
              <tr className="text-left text-[#9A968B] border-b border-[#3d3d3a] bg-[#1a1918]">
                <th className="px-[5px] py-2 font-normal">Client</th>
                <th className="px-[5px] py-2 font-normal">Name</th>
                <th className="px-[5px] py-2 font-normal">OS</th>
                <th className="px-[5px] py-2 font-normal">Status</th>
                <th className="px-[5px] py-2 font-normal">State</th>
                <th className="px-[5px] py-2 font-normal whitespace-nowrap">Current Action</th>
                <th className="px-[5px] py-2 font-normal whitespace-nowrap">Last Session</th>
            ...
            <tbody className="divide-y divide-[#3d3d3a]">
              {clientStatus.map((cs) => (
                <tr key={cs.client_id} className="hover:bg-[#1f1e1d] transition-colors">
                  <td className="px-[5px] py-2 text-[#f4f3ee] whitespace-nowrap">{String(cs.client_id).padStart(2, "0")}</td>
                  <td className="px-[5px] py-2 text-[#9A968B] whitespace-nowrap">{cs.client_name || "----"}</td>
                  <td className="px-[5px] py-2 text-[#9A968B] whitespace-nowrap">{cs.system_type || "----"}</td>
                  <td className="px-[5px] py-2 whitespace-nowrap">
                    {cs.connected ? (
                  ...
                  <td className="px-[5px] py-2 text-[#9A968B] whitespace-nowrap">
                    {!cs.connected
                      ? "------"
              ...
              <tr className="text-left text-[#9A968B] border-b border-[#3d3d3a] bg-[#1a1918]">
                ...
                {tab === "settings" && (
                  <>
                    ...
                    <th className="px-[5px] py-2 font-normal">Schedule</th>
                    <th className="px-[5px] py-2 font-normal whitespace-nowrap">Actions</th>
                    <th className="px-[5px] py-2 font-normal">Delay</th>
                    <th className="px-[5px] py-2 font-normal">Runs/Day</th>
                  </>
                )}
                {tab === "activity" && (
                  <>
                ...
                <th className="px-[5px] py-2 font-normal w-full text-right whitespace-nowrap">
                  {tab === "activity" && (
              ...
              {sortedAccounts.map((account) => {
                ...
                return (
                  <tr key={account.id} className={`hover:bg-[#1f1e1d] transition-colors ${account.system_disabled ? "text-[#5a5850]" : account.enabled ? "text-[#f4f3ee]" : "text-[#9A968B]"}`}>
                    ...
                    {tab === "settings" && (
                      <>
                        <td className="px-[5px] py-2 whitespace-nowrap">{fmtGroup(account.group_number)}</td>
                        <td className="px-[5px] py-2 whitespace-nowrap">
                    ...
                    {tab === "activity" && (
                      <>
                        <td className="px-[5px] py-2 whitespace-nowrap">{fmtNum(log?.sessions)}</td>
                        <td className="px-[5px] py-2 whitespace-nowrap">{fmtNum(log?.likes)}</td>
                        <td className="px-[5px] py-2 whitespace-nowrap">{fmtNum(log?.follows)}</td>
                        <td className="px-[5px] py-2 whitespace-nowrap">{fmtNum(log?.unfollows)}</td>
                      </>
                    )}
                    {tab === "stats" && (
                      <>
                        <td className="px-[5px] py-2 whitespace-nowrap">{fmtNum(fb?.complete)}</td>
                        <td className="px-[5px] py-2 whitespace-nowrap">{fmtNum(fb?.followed_back)}</td>
                        <td className="px-[5px] py-2 whitespace-nowrap">{fmtPct(fb?.rate ?? null)}</td>
                        <td className="px-[5px] py-2 whitespace-nowrap">{fb ? (fb.followed / (fb.days || 1)).toFixed(1) : "----"}</td>
                      </>
                    )}
                    {tab === "database" && (
                      <>
                        <td className="px-[5px] py-2 whitespace-nowrap">{fmtNum(stats?.following)}</td>
                        <td className="px-[5px] py-2 whitespace-nowrap">{fmtNum(stats?.unfollow_ready)}</td>
                        <td className="px-[5px] py-2 whitespace-nowrap">{fmtNum(stats?.complete)}</td>
                        <td className="px-[5px] py-2 whitespace-nowrap">{fmtNum(stats?.ignored)}</td>
                        <td className="px-[5px] py-2 whitespace-nowrap">{fmtNum(stats?.total)}</td>
                        <td className="px-[5px] py-2 whitespace-nowrap">{fmtNum(stats?.success)}</td>
                      </>
                    )}
                    <td className="px-[5px] py-2 text-right">
        ...
        {recentLog.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-[#9A968B] border-b border-[#3d3d3a] bg-[#1a1918]">
                  <th className="px-[5px] py-2 font-normal whitespace-nowrap">account</th>
                  <th className="px-[5px] py-2 font-normal whitespace-nowrap">date</th>
                  <th className="px-[5px] py-2 font-normal whitespace-nowrap">run</th>
                  <th className="px-[5px] py-2 font-normal whitespace-nowrap">start</th>
                  <th className="px-[5px] py-2 font-normal whitespace-nowrap">end</th>
                  <th className="px-[5px] py-2 font-normal whitespace-nowrap">action 1</th>
                  <th className="px-[5px] py-2 font-normal whitespace-nowrap">action 2</th>
                  <th className="px-[5px] py-2 font-normal whitespace-nowrap">action 3</th>
                  <th className="px-[5px] py-2 font-normal whitespace-nowrap">action 4</th>
                  <th className="px-[5px] py-2 font-normal whitespace-nowrap">errors</th>
                </tr>
              </thead>
              <tbody>
                {(() => {
