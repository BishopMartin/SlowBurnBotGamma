...
def accountSession(account, account_id, idx, threads_active, stop_flag, apiClient, permanent_idx=None):
    """
    ...
    time.sleep(1)
    print(f"- [{account}]: [setup] start thread..")
    ...
    if not settings:
        print(f"- [{account}]: [setup] ERROR: Could not fetch settings from API")
    ...
    threads_active[idx].clear()
    print(f"- [{account}]: [setup] setup complete and idle")
        ...
        if threads_active[idx].is_set():
            ...
            if driver is not None:
                try:
                    ...
                    if is_bot_debug_enabled():
                        print(f"- [{account}]: reusing existing browser session")
                except Exception:
                    driver = None
            ...
            if driver is None:
                try:
                    ...
                    drivers[account] = driver
                    if is_bot_debug_enabled():
                        print(f"- [{account}]: browser opened for session")
                except Exception as driver_error:
                    print(f"- [{account}]: ERROR: Failed to create Chrome driver: {driver_error}")
                    apiClient.log_error(account_id, f"Chrome driver creation failed: {driver_error}")
                    threads_active[idx].clear()
                    continue

            if driver is None:
                print(f"- [{account}]: ERROR: create_driver returned None - skipping session")
            ...
            try:
                ...
                if fresh_settings:
                    ...
                    if is_bot_debug_enabled():
                        print(f"- [{account}]: Re-read settings from API")
            except Exception as e:
                if is_bot_debug_enabled():
                    print(f"- [{account}]: Error re-reading settings, using cached: {e}")
            ...
            if not check_schedule(scheduleDays, scheduleStart, scheduleEnd):
                print(f"- [{account}]: Skipping - outside scheduled time/day")
                ...
                try:
                    ...
                    print(f"- [{account}]: Warning - failed to log skipped session: {e}")
                threads_active[idx].clear()
                continue
            ...
            if scheduleMax > 0:
                try:
                    ...
                    print(f"- [{account}]: Error reading run count, returning to IDLE: {e}")
                    threads_active[idx].clear()
                    continue
                if current_run_count >= scheduleMax:
                    print(f"- [{account}]: Skipping - max runs per day reached ({current_run_count}/{scheduleMax})")
                    threads_active[idx].clear()
                    continue
            ...
            try:
                if stop_flag.is_set():
                    print(f"- [{account}]: Shutdown requested, skipping login and bot script")
                    threads_active[idx].clear()
                    break
                ...
                if loginFailureExit:
                    ...
                    try:
                        ...
                        print(f"- [{account}]: Warning - failed to log login failure session: {e}")
                        run_count, max_runs = 1, scheduleMax

                    try:
                        send_login_failure_alert(account, error_msg, run_count, max_runs, apiClient=apiClient)
                    except Exception as notif_error:
                        print(f"- [{account}]: Warning - notification failed: {notif_error}")

                    threads_active[idx].clear()
                    session_already_handled = True
                    run_info = f"[{run_count}/{max_runs}]" if max_runs > 0 else f"[{run_count}]"
                    print(f"- [{account}]: returning to IDLE state due to login failure - run {run_info}")
                    continue
                ...
                if not loginFailureExit:
                    ...
                    try:
                        ...
                        if is_bot_debug_enabled():
                            print(f"- [{account}]: Ready to execute actions")
                    except Exception:
                        main_window = None
                        ...
                        if _enabled and _act_type:
                            ...
                            try:
                                if _act_type == "like" and _act_target in ["home", "homepage posts", "post[homepage]"]:
                                    ...
                                    if _topics:
                                        actions_run += 1
                                        _count, _errs = do_like_posts_topic(driver, account, _total, apiClient, account_id, _topics)
                                        if _errs:
                                            moduleErrorsLog += _errs
                                    else:
                                    ...
                                    if is_bot_debug_enabled():
                                        print(f"- [{account}]: [action {_slot_num}] ({_act_type}/{_act_target}) is a placeholder")
                                    actions_run += 1
                            ...
                            print(f"- [{account}]: [action {_slot_num}][enabled] -  [no type]")
                        else:
                            _display = _act_type if _act_type else "no type"
                            print(f"- [{account}]: [action {_slot_num}][disabled] -  [{_display}]")
                ...
                if not session_already_handled:
                    ...
                    try:
                        ...
                        print(f"- [{account}]: Warning - session notification failed: {notif_error}")
                        process_exception(printError=False, noteError="session notification failed", logError=False, debugError=False)

                    threads_active[idx].clear()
                    run_info = f"[{run_count}/{max_runs}]" if max_runs > 0 else f"[{run_count}]"
                    print(f"- [{account}]: [summary] run {run_info} - {actions_run} action(s) executed")
                ...
                if close_browser_session:
                    if driver is not None:
                        ...
                        try:
                            ...
                            if is_bot_debug_enabled():
                                print(f"- [{account}]: browser closed after session")
                        except Exception:
                            pass
            ...
            if close_browser_exit:
                ...
                try:
                    ...
                    if window_handles:
                        ...
                        if extra_windows:
                            print(f"- [{account}]: Closing {len(extra_windows)} extra browser window(s)...")
                    ...
                    driver_still_connected = False
                    print(f"- [{account}]: Driver connection already lost, will kill Chrome processes directly")
                ...
                try:
                    ...
                    time.sleep(0.5)
                    kill_chrome_processes_for_profile(chrome_user_data_dir, account)
                    print(f"- [{account}]: thread exiting, browser closed.")
                except Exception as e:
                    print(f"- [{account}]: Error killing Chrome processes: {e}")
            else:
                try:
                    print(f"- [{account}]: thread exiting, browser ready.")
                except Exception as e:
                    print(f"- [{account}]: Error during disconnect: {e}")
        except Exception as e:
            print(f"- [{account}]: Error during cleanup: {e}")
