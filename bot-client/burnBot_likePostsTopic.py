...
def _open_post_from_results(driver, account, post_url):
    ...
    try:
        ...
        if is_bot_debug_enabled():
            print(f"- [{account}]: [like][topics][debug] - could not find result tile for [{post_path}]")
        return False
    ...
    if not clicked:
        if is_bot_debug_enabled():
            print(f"- [{account}]: [like][topics][debug] - result tile click failed for [{post_path}]")
        return False
    ...
    try:
        ...
        if is_bot_debug_enabled():
            print(f"- [{account}]: [like][topics][debug] - post did not finish opening for [{post_path}]")
        return False
    ...
    try:
        ...
        if not search_clicked:
            try:
                ...
                search_clicked = True
                if is_bot_debug_enabled():
                    print(f"- [{account}]: [like][topics] - using direct search page fallback for [{topic}]")
            except Exception:
                print(f"- [{account}]: [like][topics] - [error] could not open search for [{topic}]")
                return False
        ...
        if not search_input:
            print(f"- [{account}]: [like][topics] - [error] search box not found for [{topic}]")
            return False
...
def do_like_posts_topic(driver, account, target_count, apiClient=None, account_id=None, topics=None):
    """
    ...
    if apiClient:
        try:
            ...
            if ignore_list and is_bot_debug_enabled():
                print(f"- [{account}]: [like][topics] - loaded {len(ignore_list)} ignored account(s)")
        except Exception as e:
            print(f"- [{account}]: [like][topics] - Warning: Could not load ignore list: {e}")
    ...
    try:
        for topic in topic_list:
            ...
            print(f"- [{account}]: [like][topics] - searching topic [{topic}]")
            ...
            while likes_performed < target_count and scrolls < max_scrolls:
                ...
                if not new_link_urls:
                    if scrolls == 0:
                        print(f"- [{account}]: [like][topics] - no posts found for topic [{topic}]")
                    break
                ...
                for post_url in new_link_urls:
                    ...
                    try:
                        ...
                        if not post_opened:
                            if is_bot_debug_enabled():
                                print(f"- [{account}]: [like][topics][ skip ] - could not open [{post_url}] from results")
                            continue
                        ...
                        if article_account in ignore_list:
                            ...
                            print(f"- [{account}]: [like][topics][ skip ] - [{display_name}] - [ignored]")
                            continue
                        ...
                        if not like_sponsored:
                            try:
                                ...
                                if is_ad:
                                    display_name = article_account[:15] if len(article_account) > 15 else article_account
                                    print(f"- [{account}]: [like][topics][ skip ] - [{display_name}] - [sponsored]")
                                    continue
                            except Exception:
                                pass
                            ...
                            if like_status == "Like":
                                ...
                                if clicked:
                                    try:
                                        ...
                                        print(f"- [{account}]: [like][topics][{count_formatted}/{target_formatted}] - [{display_name}] - [{topic}]")
                                        time.sleep(random.randint(6, 8))
                                    except Exception:
                                        if is_bot_debug_enabled():
                                            print(f"- [{account}]: [like][topics][ skip ] - [{display_name}] - [like state did not change]")
                                elif is_bot_debug_enabled():
                                    print(f"- [{account}]: [like][topics][ skip ] - [{display_name}] - [like click failed]")
                            else:
                                if is_bot_debug_enabled():
                                    display_name = article_account[:15] if len(article_account) > 15 else article_account
                                    state_label = like_status if like_status else "no like control"
                                    print(f"- [{account}]: [like][topics][-----] - [{display_name}] - [{topic}] - [{state_label}]")
                        ...
                        msg = str(e).split('\n')[0]
                        if is_bot_debug_enabled():
                            print(f"- [{account}]: [like][topics] - [error] {error_type}: {msg[:80]}")
        ...
        if likes_performed < target_count:
            ...
            print(f"- [{account}]: [like][topics] - {msg} ({likes_performed}/{target_count})")
            moduleErrorsLog += f"like[topics]: {msg} ({likes_performed}/{target_count})\n"
        else:
            print(f"- [{account}]: [like][topics] - complete - {likes_performed} posts liked")
