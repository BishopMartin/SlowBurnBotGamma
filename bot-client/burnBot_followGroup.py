...
def do_follow_group(driver, account, target_count, apiClient, account_id, group_type, target_accounts):
    """
    ...
    try:
        ...
        try:
            ...
            print(f"- [{account}]: [follow][{group_type}] - Warning: Could not load follow targets: {e}")
            database_names = []
        ...
        try:
            ...
            print(f"- [{account}]: [follow][{group_type}] - Warning: Could not load ignore list: {e}")
        ...
        if not target_accounts_list:
            print(f"- [{account}]: [follow][{group_type}] - ERROR: No target accounts provided")
        ...
        if driver.find_elements(By.XPATH, "//*[contains(text(), \"Sorry, this page isn't available.\")]"):
            ...
            print(f"- [{account}]: [follow][{group_type}] - ERROR: {error_msg}")
            return 0, error_msg
        ...
        if group_type == "followers[group]":
            ...
            print(f"- [{account}]: [follow][{group_type}] - ERROR: Invalid group type")
            ...
            try:
                ...
                if not user_boxes_new:
                    ...
                    try:
                        ...
                        print(f"- [{account}]: [follow][{group_type}] - reached end of list")
                        break
            ...
            for user_box in user_boxes_new:
                ...
                try:
                    ...
                    if user_name in database_names:
                        print(f"- [{account}]: [follow][{group_type}] - [ prev ] - {target_account} | {user_name}")
                        time.sleep(random.uniform(1, 1))
                        continue
                    
                    # Check if already following
                    if user_status != "Follow":
                        print(f"- [{account}]: [follow][{group_type}] - [{user_status.lower()}] - {target_account} | {user_name}")
                        time.sleep(random.uniform(1, 1))
                        continue
                    ...
                    if _is_private and _skip_private:
                        ...
                        print(f"- [{account}]: [follow][{group_type}] - [{followed_count:02d}/{target_count:02d}] - {target_account} | {user_name}")
